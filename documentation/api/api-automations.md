...
| `showToast` | Sends a real OS notification (`param`, the message -- free text, `{token}`-expanded the same way `applySessionFilename`'s template is) -- Notification Center on macOS, the Action Center on Windows -- so it reaches someone even if they aren't looking at Studio at all right now, plus an in-app toast as a second, more immediate layer for whoever already is. Never fails outright even if the OS notification can't be shown (unsupported platform, permission denied); always logged to the Connections activity feed regardless. The natural pairing is a rule set's own conditional steps (see "Sequences, delays, and running steps together" below) -- On Failure, say, for something that needs a human to go handle manually -- but nothing here requires that pairing |
