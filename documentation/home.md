...
cropped, pointed and in sync. It's optimized for running FoundryVTT sessions but not exclusive to
Foundry -- it works with any web-based experience. It signs in to Coffee Pub Tavern to publish
each player at the table as their own OBS source, and it runs a small local HTTPS server so a
Foundry module -- Coffee Pub Herald first -- can trigger an OBS action (switch scene, show or hide
a source, start or stop recording or streaming) when something happens in the game.
